...
import { sendOtp, verifyResetOtp, sendPasswordResetEmail } from "../lib/supabase.js";
...
const ResetPasswordPage = ({ onNavigate }) => {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      const { data, error: err } = await verifyResetOtp(email, verifyCode);
      if (err) {
        setError(err);
      } else {
        // OTP verified, now send reset email via Supabase
        const { data: resetData, error: resetErr } = await sendPasswordResetEmail(email);
        if (resetErr) {
          setError(resetErr);
        } else {
          setStep("success");
          setMessage("验证成功！密码重置链接已发送到您的邮箱");
        }
      }
    } catch (e) {
      setError("验证失败，请重试");
        ...
        {step === "success" && (
          <>
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              <Mail size={64} color="var(--c-success)" style={{ marginBottom: 16 }} />
              ...
              <div style={{ fontSize: 14, color: "var(--c-s400)", marginBottom: 24 }}>
                密码重置链接已发送到 <strong>{email}</strong><br />
                请查收邮件并点击链接设置新密码
              </div>
              <button
                onClick={handleBackToLogin}
