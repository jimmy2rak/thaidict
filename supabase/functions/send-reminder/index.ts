...
// Sends daily learning reminder emails with task status (modern template only)
...
const SITE_URL = 'https://thaidict.vercel.app'
...
function generateModernTemplate(tasks: Array<{ name: string; type: string; done: boolean }>, time: string): string {
  ...
  const taskItems = tasks.map(t => `
    <div style="display:flex;align-items:center;padding:11px 0;border-bottom:1px solid #2a2a2a;">
      ...
      <div style="flex:1;"><span style="font-size:14px;color:${t.done ? '#666' : '#e0e0e0'};text-decoration:${t.done ? 'line-through' : 'none'};">${t.name}</span></div>
...
<div style="max-width:420px;margin:0 auto;">
  <div style="background:#1a1a1a;border-radius:14px;overflow:hidden;border:1px solid #2a2a2a;">
    ...
    <div style="padding:0 24px 20px;text-align:center;">
      <a href="${SITE_URL}" style="display:inline-block;padding:12px 32px;border-radius:10px;background:linear-gradient(135deg,#5B8C7E,#7ab39f);color:#fff;font-size:14px;font-weight:600;text-decoration:none;letter-spacing:0.5px;">去查看</a>
    </div>
    <div style="padding:14px 24px;border-top:1px solid #2a2a2a;text-align:center;">
      <div style="font-size:12px;color:#555;">${time} CST · 词笺 · ThaiDict</div>
    </div>
  </div>
</div></body></html>`
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { email, tasks = [], time = '08:00' } = await req.json()
    ...
    const subject = total === 0
      ? '词笺 · 今日学习提醒'
      : completed === total
        ? `词笺 · 🎉 今日 ${total} 项任务已全部完成！`
        : `词笺 · 今日 ${total} 项学习任务待完成`

    const htmlContent = generateModernTemplate(tasks, time)
